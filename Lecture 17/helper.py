...
def plot_boundary(elect_train, dt1, dt2):
    ...
    grid = pd.DataFrame(np.c_[x1x.ravel(), x2x.ravel()], columns=["minority", "bachelor"])

    yhat2 = dt1.predict(grid).reshape(x1x.shape)
    yhat10 = dt2.predict(grid).reshape(x1x.shape)
